...
Created: 2026-09-22 · Status: **PLANNED — execution pending**
...
| Action | Routes |
|---|---|
| Delete/redirect | `jobs/old`, `jobs/[id]/old` → new |
...
- Redirects `next.config.ts` `redirects()` me, SEO/old-links safe
- Dead code delete → bundle + cognitive load dono kam

**IMPACT:** confusion khatam · **EFFORT:** 1 · **PRIORITY:** P1
...
| # | Item | Effort |
|---|---|---|
| 1 | `useToast` + kill 97 `alert()` | 2 |
| 2 | Semantic tokens migration + `!important` delete (codemod) | 3 |
| 3 | `ui/` kit: StatusBadge + EmptyState + ConfirmDialog + PageHeader move | 2 |
| 4 | Mobile input fundamentals (16px, dvh, safe-area) | 1 |
| 5 | `lib/jobStatus.ts` + `waLink.ts` + `useAdminGuard` | 1 |
